...
function make_options_C($var_name,$data_type,$dim,$no_args,$func_name){
  $k=1;
	$list_options[0]="";
	...
	for($j=0;$j<$k;++$j){
		...
		$list_options[$j].='){  }';
		echo '<input type="radio" name="select" value="'.$list_options[$j].'">'.'<textarea name="comments" cols="95" rows="2">'.$list_options[$j].'
		</textarea><br>';
	}
}

function make_options_java($var_name,$data_type,$dim,$no_args,$func_name){
	$k=1;
	$list_options[0]="";
	$list_options[0].=$func_name.'(';
	for($i=0;$i<$no_args;++$i){
	...
	for($j=0;$j<$k;++$j){
		$list_options[$j]= substr($list_options[$j], 0, -1);
		$list_options[$j].='){  //write code here}';
		echo '<input type="radio" name="select1" value="'.$list_options[$j].'">'.'<textarea name="comments" cols="95" rows="2">'.$list_options[$j].'
		</textarea><br>';
	}
}
...
make_options_java($var_name,$data_type,$dim,$no_args,$func_name);
